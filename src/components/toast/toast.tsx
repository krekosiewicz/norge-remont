...
// export const Toast: FC<ToastProps> = ({ message, isVisible, onClose }) => {
//   useEffect(() => {
//     if (isVisible) {
//       const timer = setTimeout(() => {
//         onClose();
//       }, 3000);  // Toast will auto-dismiss after 3000 ms (3 seconds)
//
...
export const Toast:  FC<ToastProps> = ({ type, message }) => {
  return (
    <div
      className={`fixed bottom-10 right-10 flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800`}
      role="alert"
    >
      <div
        className={`inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-green-500 bg-green-100 rounded-lg dark:bg-green-800 dark:text-green-200`}
